			...
			public MainApp ()
			{
				var menu = new MenuBar (new MenuBarItem [] {
					new MenuBarItem ("_Options", new MenuItem [] {
						new MenuItem ("_Run Worker", "", (sender, e) => RunWorker(), null, null, Key.CtrlMask | Key.R),
						null,
				...
				var statusBar = new StatusBar (new [] {
					new StatusItem(Key.CtrlMask | Key.Q, "~^Q~ Exit", (sender, e) => Application.RequestStop()),
					new StatusItem(Key.CtrlMask | Key.P, "~^R~ Run Worker", (sender, e) => RunWorker())
				});
				Add (statusBar);
			...
			public StagingUIController (DateTime? start, List<string> list)
			{
				...
				var menu = new MenuBar (new MenuBarItem [] {
					new MenuBarItem ("_Stage", new MenuItem [] {
						new MenuItem ("_Close", "", (sender, e) => { if (Close()) { Application.RequestStop(); } }, null, null, Key.CtrlMask | Key.C)
					})
				});
				top.Add (menu);

				var statusBar = new StatusBar (new [] {
					new StatusItem(Key.CtrlMask | Key.C, "~^C~ Close", (sender, e) => { if (Close()) { Application.RequestStop(); } }),
				});
				top.Add (statusBar);
