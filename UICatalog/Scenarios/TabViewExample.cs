...
namespace UICatalog.Scenarios {
	...
	public class TabViewExample : Scenario {
		...
		public override void Setup ()
		{
			...
			tabView.AddTab (new TabPage ("Tab1", new Label ("hodor!")), false);
			tabView.AddTab (new TabPage ("Tab2", new Label ("durdur")), false);
			tabView.AddTab (new TabPage ("Interactive Tab", GetInteractiveTab ()), false);
			tabView.AddTab (new TabPage ("Big Text", GetBigTextFileTab ()), false);
			tabView.AddTab (new TabPage (
				"Long name Tab, I mean seriously long.  Like you would not believe how long this tab's name is its just too much really woooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooowwww thats long",
				 ...
				 false);
			tabView.AddTab (new TabPage ("Les Mise" + Char.ConvertFromUtf32 (Int32.Parse ("0301", NumberStyles.HexNumber)) + "rables", new Label ("This tab name is unicode")), false);

			for (int i = 0; i < 100; i++) {
				tabView.AddTab (new TabPage ($"Tab{i}", new Label ($"Welcome to tab {i}")), false);
			}
		...
		{
			tabView.AddTab (new TabPage (), false);
		}
