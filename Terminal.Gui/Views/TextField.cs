...
namespace Terminal.Gui
{
	...
	public class TextField : View
	{
		...
		private void HistoryText_ChangeText(object sender, HistoryText.HistoryTextItem obj)
		{
			...
			Text = new string(obj?.Lines[obj.CursorPosition.Y].ToArray());
		...
		public new string Text
		{
			...
			set {
				var oldText = new string(text.ToArray());
				...
				if (!Secret && !historyText.IsFromHistory) {
					historyText.Add(new List<List<char>>() { oldText.ToArray().ToList() },
						new Point(point, 0));
					historyText.Add(new List<List<char>>() { text }, new Point(point, 0)
		...
		{
			Text = new string(newText.ToArray());
		}

		void SetText(IEnumerable<char> newText)
		{
		...
		void SetClipboard(IEnumerable<char> text)
		{
			if (!Secret)
				Clipboard.Contents = new string(text.ToArray());
		}
		...
		void InsertText(KeyEvent kb, bool useOldCursorPos = true)
		{
			...
			var kbstr = new List<char>() { (char)kb.Key };
			if (Used) {
				point++;
		...
		void PrepareSelection(int x, int direction = 0)
		{
			...
			if (selectedStart > -1) {
				...
				SetSelectedStartSelectedLength();
				if (start > -1 && length > 0) {
					selectedText = length > 0 ? new string(text.ToArray()).Substring(
		...
		List<char> DeleteSelectedText()
		{
			string actualText = Text;
			SetSelectedStartSelectedLength();
			int selStart = SelectedStart > -1 ? start : point;
			var newText = actualText.Remove(selStart, length);
			ClearAllSelection();
			point = selStart >= newText.Length ? newText.Length : selStart;
			return newText.ToArray().ToList();
		}
		...
		public virtual void Paste()
		{
			...
			Text = actualText.Insert(selStart, cbTxt);
			point = selStart + cbTxt.Length;
			ClearAllSelection();
			SetNeedsDisplay();
			Adjust();
		}
	...
	public class TextFieldAutocomplete : Autocomplete
	{
		...
		protected override string GetCurrentWord(int columnOffset = 0)
		{
			...
			var currentLine = host.Text;
			var cursorPosition = Math.Min(host.CursorPosition + columnOffset, currentLine.Length);
