...
namespace Terminal.Gui.TextTests
{
	public class TextFormatterTests
	{
		...
		public void CalcRect_SingleLine_Returns_1High()
		{
			...
			text = "test";
			Assert.Equal(new Rect(0, 0, text.Length, 1), TextFormatter.CalcRect(0, 0, text));
			Assert.Equal(new Rect(0, 0, text.Length, 1), TextFormatter.CalcRect(0, 0, text));
		...
		public void CalcRect_MultiLine_Returns_nHigh()
		{
			...
			text = " ~  s  gui.cs   master ↑10\n";
			lines = 2;
			Assert.Equal(new Rect(0, 0, text.Length - 1, lines), TextFormatter.CalcRect(0, 0, text));
			Assert.Equal(new Rect(0, 0, text.ToRuneList().Sum(r => Math.Max(Rn.ColumnWidth(r), 0)), lines), TextFormatter.CalcRect(0, 0, text));
			...
			Assert.Equal(new Rect(0, 0, text.Length - 4, 2), TextFormatter.CalcRect(0, 0, text));
		}
		...
		public void ClipAndJustify_Valid_Left()
		{
			...
			text = "test";
			maxWidth = 2;
			Assert.Equal(text.Substring(0, maxWidth), justifiedText = TextFormatter.ClipAndJustify(text, maxWidth, align));

			text = "test";
			maxWidth = int.MaxValue;
			Assert.Equal(text, justifiedText = TextFormatter.ClipAndJustify(text, maxWidth, align));
			Assert.True(justifiedText.Length <= maxWidth);
			Assert.True(justifiedText.Length <= maxWidth);

			text = "A sentence has words.";
			// should fit
			maxWidth = text.Length + 1;
			expectedClippedWidth = Math.Min(text.Length, maxWidth);
			justifiedText = TextFormatter.ClipAndJustify(text, maxWidth, align);
			Assert.Equal(expectedClippedWidth, justifiedText.Length);
			Assert.Equal(expectedClippedWidth, justifiedText.Length);
			Assert.True(expectedClippedWidth <= maxWidth);
			Assert.Equal(text.Substring(0, expectedClippedWidth), justifiedText);

			// Should fit.
			maxWidth = text.Length + 0;
			expectedClippedWidth = Math.Min(text.Length, maxWidth);
			justifiedText = TextFormatter.ClipAndJustify(text, maxWidth, align);
			Assert.Equal(expectedClippedWidth, justifiedText.Length);
			Assert.Equal(expectedClippedWidth, justifiedText.Length);
			...
			justifiedText = TextFormatter.ClipAndJustify(text, maxWidth, align);
			Assert.Equal(expectedClippedWidth, justifiedText.Length);
			Assert.Equal(expectedClippedWidth, Rn.StrWidth(justifiedText));
			Assert.True(expectedClippedWidth <= maxWidth);
			Assert.Equal(text.Substring(0, expectedClippedWidth), justifiedText);
		...
		public void ClipAndJustify_Valid_Justified()
		{
			...
			text = "test";
			maxWidth = 2;
			Assert.Equal(text.Substring(0, 2), TextFormatter.ClipAndJustify(text, maxWidth, align));
			...
			// should fit
			maxWidth = text.Length + 1;
			expectedClippedWidth = Math.Max(text.Length, maxWidth);
			justifiedText = TextFormatter.ClipAndJustify(text, maxWidth, align);
			Assert.Equal(expectedClippedWidth, justifiedText.Length);
			Assert.True(expectedClippedWidth <= maxWidth);
			Assert.Throws<ArgumentOutOfRangeException>(() => text.Substring(0, expectedClippedWidth));
		...
		public void Justify_Sentence()
		{
			...
			forceToWidth = text.Length;
			justifiedText = text.Replace(" ", "+");
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = "012++456+89";
			forceToWidth = text.Length + 1;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = text.Replace(" ", "++");
			forceToWidth = text.Length + 2;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = "012+++456++89";
			forceToWidth = text.Length + 3;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = text.Replace(" ", "+++");
			forceToWidth = text.Length + 4;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = "012++++456+++89";
			forceToWidth = text.Length + 5;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = text.Replace(" ", "++++");
			forceToWidth = text.Length + 6;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = text.Replace(" ", "+++++++++++");
			forceToWidth = text.Length + 20;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));

			justifiedText = "012+++++++++++++456++++++++++++89";
			forceToWidth = text.Length + 23;
			Assert.Equal(justifiedText.ToString(), TextFormatter.Justify(text, forceToWidth, fillChar).ToString());
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
			Assert.True(Math.Abs(forceToWidth - justifiedText.Length) < text.Count(" "));
		...
		public void WordWrap_SingleWordLine()
		{
			...
			Assert.Equal(text.GetRangeEx(0, text.Length - 1).ToString(), wrappedLines[0].ToString());
			Assert.Equal("e", wrappedLines[1].ToString());

			width = text.Length - 2;
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(2, wrappedLines.Count);
			Assert.Equal(text.GetRangeEx(0, text.Length - 2).ToString(), wrappedLines[0].ToString());

			width = text.Length - 5;
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(2, wrappedLines.Count);

			width = (int)Math.Ceiling((double)(text.Length / 2F));
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(2, wrappedLines.Count);
			Assert.Equal("Constan", wrappedLines[0].ToString());
			Assert.Equal("tinople", wrappedLines[1].ToString());

			width = (int)Math.Ceiling((double)(text.Length / 3F));
			wrappedLines = TextFormatter.WordWrap(text, width);
			...
			width = (int)Math.Ceiling((double)(text.Length / 4F));
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(4, wrappedLines.Count);

			width = (int)Math.Ceiling((double)text.Length / text.Length);
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(text.Length, wrappedLines.Count);
			Assert.Equal(text.Length, wrappedLines.Count);
		...
		public void WordWrap_Unicode_SingleWordLine()
		{
			...
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(2, wrappedLines.Count);
			Assert.Equal(text.Substring(0, text.Length - 1), wrappedLines[0].ToString());
			Assert.Equal("ำ", wrappedLines[1].ToString());

			width = text.Length - 2;
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(2, wrappedLines.Count);
			Assert.Equal(text.Substring(0, text.Length - 2), wrappedLines[0].ToString());
			...
			wrappedLines = TextFormatter.WordWrap(text, width);
			Assert.Equal(text.Length, wrappedLines.Count);
			Assert.Equal(Rn.StrWidth (text), wrappedLines.Count);
...
#pragma warning restore xUnit2013 // Do not use equality check to check for collection size.
		...
		public void WordWrap_NoNewLines()
		{
			...
			Assert.True(wrappedLines.Count == 1);
			Assert.True(expectedClippedWidth >= wrappedLines.Max(l => l.Length));
			Assert.True(expectedClippedWidth >= wrappedLines.Max(l => l.Length));
			...
			maxWidth = text.Length - "words.".Length;
			expectedClippedWidth = Math.Min(text.Length, maxWidth);
			wrappedLines = TextFormatter.WordWrap(text, maxWidth);
			Assert.Equal(2, wrappedLines.Count);
			Assert.True(expectedClippedWidth >= wrappedLines.Max(l => l.Length));
			...
			maxWidth = text.Length - "s words.".Length;
			expectedClippedWidth = Math.Min(text.Length, maxWidth);
			wrappedLines = TextFormatter.WordWrap(text, maxWidth);
			Assert.Equal(2, wrappedLines.Count);
			Assert.True(expectedClippedWidth >= wrappedLines.Max(l => l.Length));
			...
			maxWidth = text.Length - "Ð²ÐµÑ) has words.".Length;
			expectedClippedWidth = Math.Min(text.Length, maxWidth);
			wrappedLines = TextFormatter.WordWrap(text, maxWidth);
			Assert.Equal(2, wrappedLines.Count);
			Assert.True(expectedClippedWidth >= wrappedLines.Max(l => l.Length));
...
#pragma warning restore xUnit2013 // Do not use equality check to check for collection size.
		...
		public void WordWrap_preserveTrailingSpaces_Wide_Runes()
		{
			...
			Assert.Equal(25, Rn.StrWidth(text));
		}
...
";
		...
		public void ReplaceHotKeyWithTag()
		{
			...
			Assert.Equal(new string(new char[] { tag, 'e', 's', 't' }), tf.ReplaceHotKeyWithTag(text, hotPos));

			tag = 'e';
			hotPos = 1;
			Assert.Equal(new string(new char[] { 't', tag, 's', 't' }), tf.ReplaceHotKeyWithTag(text, hotPos));

			var result = tf.ReplaceHotKeyWithTag(text, hotPos);
			Assert.Equal('e', result.ToCharArray()[1]);

			text = "Ok";
			tag = 'O';
			hotPos = 0;
			Assert.Equal(new string(new char[] { tag, 'k' }), result = tf.ReplaceHotKeyWithTag(text, hotPos));
			Assert.Equal('O', result.ToCharArray()[0]);

			text = "[◦ Ok ◦]";
			text = new string(new char[] { '[', '◦', ' ', 'O', 'k', ' ', '◦', ']' });
			var runes = text.ToCharArray();
			Assert.Equal(text.Length, runes.Length);
			Assert.Equal(text, new string(runes));
			tag = 'O';
			hotPos = 3;
			Assert.Equal(new string(new char[] { '[', '◦', ' ', tag, 'k', ' ', '◦', ']' }), result = tf.ReplaceHotKeyWithTag(text, hotPos));
			Assert.Equal('O', result.ToCharArray()[3]);

			text = "^k";
			tag = '^';
			hotPos = 0;
			Assert.Equal(new string(new char[] { tag, 'k' }), result = tf.ReplaceHotKeyWithTag(text, hotPos));
			Assert.Equal('^', result.ToCharArray()[0]);
		}
		...
		public void Reformat_NoWordrap_SingleLine()
		{
			...
			list = TextFormatter.Format(text, maxWidth, TextAlignment.Left, wrap);
			Assert.True(list.Count == 1);
			Assert.Equal(text.Substring(0, expectedClippedWidth), list[0]);
		...
		public void Reformat_NoWordrap_NewLines()
		{
			...
			list = TextFormatter.Format(text, maxWidth, TextAlignment.Left, wrap);
			Assert.True(list.Count == 1);
			Assert.Equal(text.Substring(0, expectedClippedWidth).Replace("\n", " "), list[0]);
			...
			maxWidth = text.Length - 1;
			expectedClippedWidth = Math.Min(text.Length, maxWidth) + 1;
			list = TextFormatter.Format(text, maxWidth, TextAlignment.Left, wrap);
			Assert.True(list.Count == 1);
		...
		public void Reformat_Wrap_Spaces_No_NewLines()
		{
			...
			expectedClippedWidth = text.Length - text.Sum(r => r == ' ' ? 1 : 0);
			list = TextFormatter.Format(text, maxWidth, TextAlignment.Left, wrap);
			...
			expectedClippedWidth = text.Length;
			...
			maxWidth = 5;
			// remove 3 whitespace chars
			expectedClippedWidth = Math.Min(text.Length, maxWidth - text.Sum(r => r == ' ' ? 1 : 0));
			list = TextFormatter.Format(text, maxWidth, TextAlignment.Left, wrap);
			Assert.Equal(expectedClippedWidth, list.Count);
			Assert.Equal(text.Substring(0, expectedClippedWidth), list[0]);
			Assert.Equal("01245689", string.Join("", list.ToArray()));
			...
			maxWidth = 5;
			// keep 3 whitespace chars
			expectedClippedWidth = Math.Min(text.Length, (int)Math.Ceiling((double)(text.Length / 3F)));
			list = TextFormatter.Format(text, maxWidth, TextAlignment.Left, wrap, preserveTrailingSpaces);
...
", output);
		...
		public void GetTextWidth_Simple_And_Wide_Runes()
		{
			...
			Assert.Equal(11, Rn.StrWidth (text));
			text = "こんにちは世界";
			Assert.Equal(14, Rn.StrWidth (text));
		}
		...
		{
			Assert.Equal(6, TextFormatter.GetMaxLengthForWidth("Hello World", 6));
			Assert.Equal(3, TextFormatter.GetMaxLengthForWidth("こんにちは 世界", 6));
			Assert.Equal(15, TextFormatter.GetMaxLengthForWidth("[ Say Hello 你 ]", 16));
		}
...
";
		...
		public void MaxWidthLine_With_And_Without_Newlines()
		{
			...
			Assert.Equal(13, TextFormatter.MaxWidthLine(text));
		...
		public void Non_Bmp_ConsoleWidth_ColumnWidth_Equal_Two()
		{
			...
			Assert.Equal(2, Rn.ColumnWidth(r));

			var win = new Window(us);
			var label = new Label(us);
